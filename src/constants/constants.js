export const projects = [
  {
    title: 'MLBB WR Counter',
    description:"To find out the estimate of how many matches you need to get the Win Rate you want.",
    image: '/images/2.png',
    tags: ['Bootstrap', 'JavaScript'],
    source: 'https://github.com/dodepersie/penghitung-wr-mlbb',
    visit: 'http://penghitung-wr-mlbb.vercel.app/',
    id: 0,
  },
  {
    title: 'NextJS Portfolio',
    description: "You're on this portfolio site now! Mahadi Saputra'\s second portfolio website created with NextJS!",
      image: '/images/4.png',
      tags: ['AOS', 'NextJS'],
    source: 'https://github.com/dodepersie/nextjs-portofolio',
    visit: 'https://mahadisaputra.my.id/',
    id: 1,
  },
  {
    title: 'ReactJS Portfolio',
    description: "Mahadi Saputra'\s first and complete portfolio website created with ReactJS, Bootstrap and AOS Library!",
      image: '/images/3.png',
      tags: ['AOS', 'Bootstrap', 'React'],
    source: 'https://github.com/dodepersie/landing',
    visit: 'https://v1.mahadisaputra.my.id/',
    id: 2,
  },
  {
    title: 'TMDB MovieAPI',
    description: "See popular movies, see playing movies on cinema and search for movies! API powered by TMDB",
      image: '/images/1.png',
      tags: ['API', 'Bootstrap', 'React'],
    source: 'https://github.com/dodepersie/movie-api-by-tmdb',
    visit: 'http://movie-api-by-tmdb.vercel.app/',
    id: 3,
  },
];

export const TimeLineData = [
  { year: 2012, text: 'Started my journey by making a technology blog', },
  { year: 2013, text: 'Developing a Twitter exchange followers website', },
  { year: 2014, text: 'Selling Twitter followers', },
  { year: 2019, text: 'Going to information system college', },
  { year: 2022, text: 'Learning more about programming language', },
];